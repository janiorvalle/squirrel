...
- **nuke**. Use only when the human types /nuke. Throws away the worktree you're in: stop what you started for it, release the lock, remove the tree, delete the local branch, report, hold. Refuses if the branch has commits no remote has. The undo of worktree.
