...
- **nuke**. Use only when the human types /nuke. Tears down one worktree and stops: its compose stack with volumes, its lock, the tree, the local branch. Refuses if the branch has commits the remote doesn't. The undo of worktree.
