...
- Enter another agent's worktree. A lead reads a worker's branch from its own checkout with `git fetch` and `git diff origin/main...origin/<branch>`, and builds it in a worktree of its own; a `cd` into someone else's tree is how a stray commit lands on their branch.
